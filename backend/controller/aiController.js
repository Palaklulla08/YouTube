...
// function for searching with ai
export const searchWithAi = async (req, res) => {
  try {
    // getting input from req.body
    const { input } = req.body;
    // if no input return a message required query for search
    if (!input) {
      return res.status(400).json({ message: "Search query is required" });
    }

    // ✅ Step 1: AI se keyword nikalo (autocorrect + simplified)
    // requiring api of googlegenai
...
// using gemini-2.5-flash model for ai integartion and response
