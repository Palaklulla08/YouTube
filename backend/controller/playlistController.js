...
// Create playlist function gets title description channelid videoId from req.body 
export const createPlaylist = async (req, res) => {
  try {
    const { title, description, channelId, videoIds } = req.body;
    // if there no title or channel id return  Playlist title and channel ID are required
    if (!title || !channelId) {
    ...
    // if not exist tehn returning Channel not found
    if (!channel) {
...
// finding playlist by playlistId
...
// if found then send playlist  res 
